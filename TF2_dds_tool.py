...
l_2048  = int('200000',16)
l_1024  = int('80000',16)
l_512   = int('20000',16)

ln_2048 = int('400000',16)
ln_1024 = int('100000',16)
ln_512  = int('40000',16)

length_list = [l_2048, ln_2048, l_2048, l_2048, l_2048,
               l_1024, ln_1024, l_1024, l_1024, l_1024,
               l_512,  ln_512,  l_512,  l_512,  l_512]
...
files_list = files_2048 + files_1024 + files_512
...
def apply_dds(dds_data, starpak_data, offset, length):
    """
    ...
    ----------
    dds_data : Memoryview
        memoryview of a DDS file.
    starpak_data : 
        memoryview of a starpak file.
    offset : Int
        interger corresponding to the offset where the texture starts.
    length : Int
        length of the texture and the dds content.

    Returns
    -------
    String
        string of numbers corresponding to hex values of a starpak file.

    """
    starpak_data[offset:offset + length] = dds_data
    return starpak_data
...
def read_starpak():
    ...
    """
    with open(pc_stream,'rb') as starpak:
        starpak_data = memoryview(bytearray(starpak.read()))
    return starpak_data
...
def read_dds(filename):
    ...
    with open(filename,'rb') as ddsfile:
        dds_data = memoryview(bytearray(ddsfile.read()))[128:]
    return dds_data

def write_starpak(starpak_data):
    """
    ...
    """
    with open('NEW_pc_stream.starpak','wb') as output:
        output.write(starpak_data)
    return 'Complete'
    


def main_function(stardata, lof, loo, lol):
    """
    ...
    """
    if lof == []:
        return write_starpak(stardata)
    elif lof[0] == 'pass':
        main_function(stardata, lof[1:], loo[1:], lol[1:])
    else:
        acc = apply_dds(read_dds(lof[0]), stardata, loo[0], lol[0])
        main_function(acc, lof[1:], loo[1:], lol[1:])
...
def confirm_selection(selected):
    ...
    while option != 0:
        if option == 'y':
            clear()
            print(main_function(read_starpak(), files_list, 
                                selected['offsets'], length_list))
            option = 0
        elif option == 'n':
            clear()
...
def menu(message):
    ...
    while option != 0:
        if option == 1:
            ...
            clear()
            menu("Invalid selection, try agian -> ")
            option = 0
